...
import { ClaudeCodeService } from '../anthropic/claude-code.service';
...
export class AgentProcessor {
  ...
  constructor(
    ...
    this.services = {
      ...
      "claude-code": this.claudeCodeService,
  ...
  private async runIteration(taskId: string): Promise<void> {
    ...
    try {
      ...
      for await (const chunk of service.generateMessage(
        AGENT_SYSTEM_PROMPT,
        messages,
        model.name,
        true,
      ...
      if (!agentResponse) {
        this.logger.warn(
          `Task ID: ${taskId} received no response from LLM, marking as failed`,
        );
        await this.tasksService.update(taskId, {
          status: TaskStatus.FAILED,
        });
      ...
      if (shouldSummarize) {
        try {
          ...
          let summaryResponse: BytebotAgentResponse | undefined;
          for await (const chunk of service.generateMessage(
            SUMMARIZATION_SYSTEM_PROMPT,
            [
              ...messages,
          ...
          if (summaryResponse) {
            ...
            this.logger.log(
              `Generated summary for task ${taskId} due to token usage (${summaryResponse.tokenUsage.totalTokens}/${contextWindow})`,
            );
          }
        } catch (error: any) {
          this.logger.error(
      ...
      } else if (agentResponse?.stopReason === 'tool_code') {
        // If the agent stopped because it generated tool code, we need to continue processing
        // in the next iteration to execute the tool code.
        setImmediate(() => this.runIteration(taskId));
      } else if (agentResponse?.stopReason === 'error_max_turns') {
        await this.tasksService.update(taskId, {
          status: TaskStatus.NEEDS_HELP,
        });
        this.isProcessing = false;
        this.currentTaskId = null;
      } else if (agentResponse?.stopReason === 'error_during_execution') {
        await this.tasksService.update(taskId, {
          status: TaskStatus.FAILED,
        });
        this.isProcessing = false;
        this.currentTaskId = null;
      } else if (agentResponse?.stopReason === 'success') {
        await this.tasksService.update(taskId, {
          status: TaskStatus.COMPLETED,
          completedAt: new Date(),
        });
